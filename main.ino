...
void setup(){
  ...
  radio.setRetries(2, 2);    //(время между попыткой достучаться, число попыток)
...
void identification(){
  ...
  EEPROM.get(0, id); EEPROM.commit(); EEPROM.get(2; queue_delay); EEPROM.commit();
     ...
     while (true){
       ...
       if (radio.available()){
         ...
         if (incoming_identification_request == 1111){
           ...
           radio.openWritingPipe(address[2]);//актуально будет поменять трубу для телеметрии
           radio.stopListening();
           delay(queue_delay);
